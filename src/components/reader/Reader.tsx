    ...
    >
      ...
      <div className="pt-[calc(4rem+env(safe-area-inset-top))]">
        {/* **The resume suggestion, as one floating pill.**
            ...
            **In the flow of the page rather than floating over it** — tried
            both ways round. Floating cost the chapter's first line, which it
            covered until dismissed, and on a chapter that opens with a list
            that meant hiding the first item. In flow it costs a shift instead,
            and a shift at the top of a page nobody has begun reading is the
            cheaper of the two.

            What made floating tempting was that the old banner's shift was
            ugly — but most of that was its size, not the shifting. A
            full-width box moved the text by its whole height; a 48px pill that
            centres itself moves it by less and reads as one object arriving
            rather than a bar unrolling across the page.

            `data-reader-chrome` so a tap on it is not read as a tap on the
            page, and so the reading surfaces leave its ink alone. It is a
            one-time offer rather than furniture: it stays until it is taken or
            dismissed, and it does not hide with the chrome. */}
        {resumeHint && (
          <div data-reader-chrome className="reader-content flex justify-center pt-2">
            <div className="flex w-fit max-w-full items-center rounded-full bg-overlay p-1 text-white shadow-raised">
              <button
                type="button"
                onClick={() => {
              ...
              >
                {/* "Resume · chapter 1", not "Resume where you left off —
                    ...
                    wanted 355 of a 375px screen, so the thing read as a bar
                    with round ends. It also says twice over what the reader can
                    see — they are holding the book, so *where* they left off is
                    the only question, and the chapter answers it.
                ...
                <span className="truncate">
                  {resumeHint.kind === "other-device" ? "Other device" : "Resume"}
                  {" · chapter "}
                  {resumeHint.chapter}
                </span>
