...
import { DocumentRow } from "@/components/resources/DocumentRow";
import { EmptyState, PageContainer } from "@/components/ui";
import { getDocuments, getFolders } from "@/lib/api";
import type { Folder, ResourceDocument } from "@/lib/types";
...
export const metadata: Metadata = {
  ...
  description: "Shivir notes, presentations, Shodh Patra, Yojana and education materials.",
};

/**
 * The Resources library — a folder tree of files, browsed one level at a time.
 *
 * Not a books shelf, and deliberately not filtered by genre or language chips:
 * what organizes this material is where it sits, so the control is the trail
 * you walked to get here. Nothing on this screen leads into the book reader.
 *
 * The whole ancestry rides in the URL (`/resources/12/45`) because a folder
 * only knows its own ancestors' names, not the ids needed to link back to
 * them — and because a path is worth sharing.
 */
export default async function ResourcesPage({
  ...
  const trail = (current?.breadcrumb ?? []).map((a, i) => ({
    name: a.name,
    href: `/resources/${ids.slice(0, i + 1).join("/")}`,
  }));

  // The root blurb describes the library as a whole, so it must not stand in
  // for a folder that simply hasn't been given a description of its own.
  const blurb = current
    ? current.description
    : "शिविर सामग्री, प्रस्तुतियाँ, शोध पत्र, योजना एवं शिक्षा सामग्री।";
  ...
  return (
    <PageContainer>
        ...
        <ol className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
          <li>
            {current ? (
              ...
              <span lang="hi" className="hi font-medium">संसाधन</span>
            )}
          </li>
          {trail.map((a) => (
      ...
      <h1 lang="hi" className="hi mt-2 text-xl font-bold">
        {current ? current.name : "संसाधन · Resources"}
      </h1>
      {blurb && (
 ...
 *
 * `document_count` is published-only, so it is safe to print. `folder_count`
 * is not: it counts every child folder, including the ones the list itself
...
function countLabel(f: Folder): string {
  ...
  if (f.document_count > 0) parts.push(`${f.document_count} फ़ाइलें`);
  return parts.join(" · ");
}
