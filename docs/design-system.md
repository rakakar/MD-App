...
| Card 2 as a dark stage of six covers under Books / Audio / Video tabs | Two resume rails on light paper — books, and recordings including a video | The designer's revision, 10 Sep. The shelf-as-an-object said what Originals *is*; the two resume cards say what it is for, which is the card's own sentence: books and forty hours of his own voice, in one place. Both are the real cards — `ContinueReading`'s row and `ContinueAv`'s. The `.stage-ink` token the dark panel needed went with it. |
