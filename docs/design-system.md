...
| Reader body text set ragged-right | Justified — paragraphs only; headings, lists, contents rows, verse and captions keep their alignment | Browsers cannot hyphenate Devanagari, so on a ~6-word phone line the ragged edge swings by a whole long word and reads as broken. Hindi print is justified; the designer compared both on device (26 Sep 2026) and chose it. The cost is the odd loose line where one long word would not fit. |
| Reader book text in Tiro Devanagari at 18px, 21.6px gutters, line height 2.0 (spec) | Mukta at ~21.6px (scale 1.2), 16px gutters, Compact 1.85 line height; Tiro one tap away as "Serif" | Readers are mostly past 40. Tiro's hairlines — matras, anusvara, half-forms — are what blur first on a phone; Mukta's even stroke and larger letter body reads larger than Tiro at the same size. First set at 1.05, raised two steps on device; the cost is ~6 words a line on a phone and more loose justified lines. The designer's call, 26 Sep 2026. New readers only: saved preferences are left as chosen. |
...
| `Intl` short months | A written-down three-letter table, now `lib/dates.ts` | `en-IN` and `en-GB` abbreviate September to "Sept" — four letters where the other eleven have three — and the comps print "5 Sep'26". On the card, date and location share a line at the largest text size, and the odd month out is the one that wraps. **This rule is app-wide, not an Events one**: the table lived in `lib/events.ts` while two other surfaces reinvented it — the sutra card reassembled `Intl.formatToParts` under `en-US` to reach the same three letters, and a saved highlight asked `en-IN` and printed "12 Sept 2026", the only place in the app showing the four-letter form. All three read `MONTH_SHORT` from `lib/dates.ts` now. |
...
| Comp | Shipped | Reason |
|---|---|---|
| Card 1 lists Explore · Originals · Resources · Community · My Journey, the current one a filled dark row | Originals · Translations · Resources · My Journey · Connect, drawn as `Header`'s `sheetRow` draws them | Two things. The names are an earlier scheme this app has not had for some time — there is no Explore workspace and no Community one — and reading them out of `WORKSPACES` means the card follows the next rename without anyone remembering to. And the rows are now the bottom sheet's own: each workspace's gradient tile with its glyph, and the row you are in marked the way the sheet marks it — its accent as a border, a 7% wash behind, and a check. A filled dark row is a shape the app does not have anywhere else. |
...
| Card 2 as a dark stage of six covers under Books / Audio / Video tabs | `BookRail` with the real covers, over `ContinueAv`'s resume cards | The designer's revision, 10 Sep. The shelf-as-an-object said what Originals *is*; the two resume cards say what it is for, which is the card's own sentence: books and forty hours of his own voice, in one place. Both are the real cards — `ContinueReading`'s row and `ContinueAv`'s. The `.stage-ink` token the dark panel needed went with it. |
| Real covers and a real video still on card 2 | `CoverTile`'s designed fallback, and `ContinueAv`'s own glyph tile | Both arrive over the network. The deck opens before anything has been fetched and has to work offline, so a fragment may only show what the app knows without asking the server. Each is the real component's real fallback, not a stand-in invented here. |
...
| Both surfaces drawn on light paper | Adopted, and *forced* — `data-theme="light"` on the deck and on the mark | They are seen once, so a theme-correct dark version of each is a second design to draw, review and keep for a screen nobody meets twice. Cheap rather than contradictory because the app's own default is light now (`DEFAULT_PREFS.theme`); the only reader this can surprise is one who chose dark deliberately and then cleared their storage. `[data-theme="light"]` in `globals.css` is the one place the light palette is written twice — kept directly under the dark block so the two are edited together. It also re-declares `color`, because inheritance passes a *resolved* colour rather than the `var()` behind it, and without that every element with no colour class of its own came out near-white on white. |
...
| Buttons under the preview | In the sheet's `footer` | Not a design change — a misuse of `Sheet`, fixed. Its footer sits outside the scroller precisely because "a sheet that ends in a decision must not scroll its own decision off screen", and in the body they did: on an iPhone with Safari's own bars up, Share and Save were below the fold of an 85dvh panel. The preview is also sized by height (`h-[min(55dvh,26rem)]`) rather than width, so it can never push them away again; 3:4 means the width follows at 312px, which fits the narrowest phone this app is drawn for. |
...
| The founder's card at 82% white | 88% with `backdrop-blur-sm` | The study drew it over a patch of table. On a 768pt portrait tablet the crop puts a book spine behind it and the spine's title — Devanagari, at about the same size — read through the card and collided with the line printed on it. Opacity alone would have punched the card out of the photograph; blurring what is behind keeps it sitting *on* the picture and works wherever the artwork lands. |
...
| The phone scrim's top at 55% / 26%, clear by 28% | 82% / 68% / 42% / 16%, clear by 54% | The designer's call, 13 Sep, and the new artwork is why: its sky is a saturated sunrise where the study's was pale, so the title and the sentence under it were sitting on colour rather than on paper. The fade now runs down to the mountain line and the photograph starts where the picture actually starts. Description 5.67:1 → 6.42:1, eyebrow 6.33 → 7.33. |
| The desktop scrim at 96% / 86% / 40% | 84% / 70% / 30%, and the transparent end moved out to 62% | At the study's strength the whole left third of the photograph was washed to near-paper and the identity column sat on flat cream — the artwork stopped at the window frame. Softened until the greenery and the plant read through it, and measured rather than judged: the description over it is 6.45:1 at 1440 and 6.86:1 at 2000, against a 4.5:1 floor. |
| Eyebrow 11.5/12.5px, description 13.5/16.5px, founder card 15.75/17.5px and 12.6/14px (phone/desktop) | 14/15px, 16/20px, 18/22.5px and 15/17px | The designer's call, 12 Sep. The studies set both a step under the app's own scale — 11.5px is below `--text-xs`, which is the floor everywhere else — and on a real screen at arm's length the sentence that says what the app *is* was reading as a caption. The description now sits at `--text-sm` on the phone, the app's body step, and one step above it on the desktop, where it has a 452px column to fill. The founder's two lines went up with them — Devanagari carries matras above and below and loses more than Latin does at a small size, and this is the one line on the screen that is a proper name. Both still set on one line each, and 18px on the phone is a ceiling rather than a preference: the line measures 185px at 18/600 and 191 at 19, and a 360pt phone leaves it exactly 191. |
| Founder card in Tiro at regular weight, description at regular weight, eyebrow in `--color-muted` and spaced like the sentence below it | Mukta, `font-medium`, `--color-ink-soft`, and set tight under the title | The designer's call, 12 Sep. Tiro is the book face and it earns a chapter; two lines of caption on a card are a label, and Mukta is what the system already uses for Devanagari UI text (`.hi-note`). The first line is set at 600, which is a real face — `layout.tsx` loads Mukta at 400/500/600/700 — rather than a browser synthesising one, the trap Tiro sets by shipping a single weight. The family is set in `.launch-founder-*` rather than by adding `.hi-note` at the call site, because those classes already own the size and the leading and two classes disagreeing about one line is how twenty inert `leading-*` utilities on `.hi` elements happened elsewhere in this app. The title and eyebrow are now one unit with the sentence separate: spaced equally they read as three announcements rather than a name and what qualifies it. |
...
| Landing · Book search chosen (added 19 Sep, designer's ask) | An "All books ▾" button beside the Book search pill opens a sheet: All books, then the Originals grouped by series (the BE's genres, in its order) with an "All Darshan"-style shortcut per group; choices are a draft until the footer button | Replaced a sideways-scrolling chip row the same day — it hid ten of twelve titles off the edge. Several books are searched one call each and merged, since `search` narrows to one `book`; each book keeps its own full ranked list, at one search per book. |
...
| Composer · हिं·EN switch in the box | Removed | The designer's call, 18 Sep. Roman letters are always read as Hindi; Book search's "search as typed" link is the way back for an English word. The voice sheet keeps its own हिंदी / English choice. |
...
| Next-shivir date chip under the rail's workspace switcher | Gone | 16 Sep, and it follows the same decision that took it off the phone's app bar: its job was mitigation for Connect being a couple of taps away, a bare date is the least it could have said, and the shivirs are on Home under a heading with a place and a name — which is what a reader needs to decide whether to go. The rail was the last thing rendering `EventChip`, so the component, its fetch and its `header_event_chip_tap` event went with it. |
| Reader chrome at 95% over an 8px backdrop blur | Opaque, no blur | 16 Sep, from an iPhone 14 Plus on iOS 27. WebKit does not move an element's backdrop region with its `transform`: the bar slides off the top of the screen and the blur it was applying stays behind, sampling and smearing the strip of text the bar used to cover — two lines of Devanagari permanently out of focus under the status bar, with no bar anywhere in sight. `opacity: 0` does not stop it. At 95% the blur was doing nothing visible anyway. `.reader-chrome[data-hidden]` now also sets `backdrop-filter: none`, so the class refuses to carry one while hidden whatever a caller does later — the bug is in the pattern, not in one component. |
| App bar at 85% over a 24px backdrop blur | Opaque `--ws-chrome`, no blur | 16 Sep. The translucency was there so a little of what passes beneath would read as going *under* the bar, and over the pale shelves it was drawn against that is true and nearly invisible. Over the Shorts rail's black tiles it is neither: 15% of black turns a cream bar grey and the blur smears it as the rail scrolls, so the bar looks out of focus rather than deep. Worse on a phone, where `viewport-fit=cover` puts the status bar inside the element — nothing scrolls under a notch, so there the wash reads as a rendering fault. Depth is carried by the bottom rule and by content disappearing at it. Dropping the blur also drops a compositing layer from every scrolled frame. The workspace sheet stays portalled: `filter`, `transform`, `perspective` and `will-change` would each make the header a containing block again, and a portal is immune to all four. |
