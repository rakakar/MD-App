...
| Landing · Book search chosen (added 19 Sep, designer's ask) | A scrolling row of the Originals under the help line: "All books" (default) and each title, several at once | One row, not wrapped — twelve titles would stand four rows high. Several books are searched one call each and merged, since `search` narrows to one `book`; that gives each book its own full ranked list, at one search per book. |
